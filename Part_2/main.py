...
class GameLauncher(tk.Tk):
    ...
    def create_widgets(self):
        ...
        launch_game_button = tk.Button(
            self, text="Launch Game", command=self.launch_game
        )
        launch_game_button.pack(pady=10)

        launch_green_screen = tk.Button(
            self, text="Launch Green Screen", command=self.launch_green_screen
        )
        launch_green_screen.pack(pady=10)

        launch_object_detector = tk.Button(
            self, text="Launch Object Detector", command=self.launch_object_detector
        )
